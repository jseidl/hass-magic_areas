...
class MagicArea(object):
    ...
    def is_sleeping(self):
        if self.has_feature(CONF_FEATURE_LIGHT_GROUPS) and self.feature_config(
            CONF_FEATURE_LIGHT_GROUPS
        ).get(CONF_SLEEP_ENTITY):

            sleep_entity = self.hass.states.get(
                self.feature_config(CONF_FEATURE_LIGHT_GROUPS).get(CONF_SLEEP_ENTITY)
            )
            if (
                sleep_entity.state.lower()
                == self.feature_config(CONF_FEATURE_LIGHT_GROUPS)
                .get(CONF_SLEEP_STATE)
                .lower()
            ):
                _LOGGER.info(
    ...
    def is_night(self):
        ...
        if self.has_feature(CONF_FEATURE_LIGHT_GROUPS) and self.feature_config(
            CONF_FEATURE_LIGHT_GROUPS
        ).get(CONF_DARK_ENTITY):
            night_entity = self.hass.states.get(
                self.feature_config(CONF_FEATURE_LIGHT_GROUPS).get(CONF_DARK_ENTITY)
            )
            if night_entity and (
